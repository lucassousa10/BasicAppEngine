...
public class M extends AbstractApplication {
    ...
    public M() {
        ...
        clef = new Shape(d, 0, 0, 1f);
    }

    @Override
    public void update(Engine engine, float deltaTime) {
        if (Input.isKey(KeyEvent.VK_D)) {
            clef.transform(AffineTransform.getTranslateInstance(speed * deltaTime, 0));
    ...
    public void render(Engine engine, Renderer renderer) {
        renderer.fillShape(clef, Color.WHITE);
    }

    public static void main(String[] args) {
        Engine e = new Engine(new M());
        e.setSize(800, 800);
        e.start();
    }
}
